...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!genAI) {
      console.error(
        "Server Error: GoogleGenerativeAI was not initialized. GEMINI_API_KEY is likely missing or invalid.",
      )
      return NextResponse.json(
        { error: "Server configuration error: AI service is not properly configured." },
        { status: 500 },
      )
    }
...
User question: ${message}`
    ...
    if (error.status) {
      ...
      if (statusCode === 429) {
        ...
        errorMessage =
          "Authentication error with AI service. Check your API key and project permissions in Google Cloud Console."
      } else if (statusCode === 400) {
        errorMessage = "Bad request to AI service. Ensure your input is valid or prompt is not too long."
      } else if (statusCode === 404) {
        // Explicitly handle 404 for model not found
        errorMessage =
          "AI model not found or not available. This might be due to an incorrect model name or regional availability."
      } else {
